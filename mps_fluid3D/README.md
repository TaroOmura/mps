...
- **ハイブリッド摩擦モデル**: 接触線での静摩擦・動摩擦 (Hattori & Koshizuka 2019)
- 近傍粒子探索
- CSV形式 / VTK形式での結果出力
...
│   ├── gen_wetting.py           # 液滴濡れ広がり問題の初期条件生成スクリプト
│   └── gen_rolling.py           # 傾斜面液滴転がり問題の初期条件生成スクリプト
├── vis/
...
│   │   ├── params.txt
│   │   └── particles.txt
│   ├── wetting/                 # 液滴濡れ広がり問題の入力ファイル (gen_wetting.py で生成)
│   └── rolling/                 # 傾斜面液滴転がり問題の入力ファイル (gen_rolling.py で生成)
├── Makefile
├── .gitignore
└── README.md
```
...
床 (XZ平面) 上に液滴を落下・濡れ広がりさせる問題です。

```bash
python3 tools/gen_wetting.py \
  --l0 0.0001 \              # 粒子間距離 [m]
  --wetting_angle_SL 60.0 \  # 静的接触角 [deg] (小さいほど親水)
  --outdir examples/wetting
```

#### 傾斜面液滴転がり問題

重力ベクトルを回転させて傾斜を表現します（床は水平のまま）。摩擦モデルを組み合わせて液滴の保持・滑りを再現できます。

```bash
python3 tools/gen_rolling.py \
  --l0 0.0001 \                    # 粒子間距離 [m]
  --incline_angle 60.0 \           # 傾斜角 [deg]
  --wetting_angle_SL 120.0 \       # 静的接触角 [deg] (>90: 撥水)
  --friction_enabled 1 \           # ハイブリッド摩擦モデル ON
  --friction_delta_theta 30.0 \    # 静摩擦: 臨界接触角偏差 Δθ_c [deg]
  --dynamic_friction_alpha 600.0 \ # 動摩擦: 抵抗係数 α
  --friction_velocity_threshold 0.01 \  # 切り替え臨界速度 v_c [m/s]
  --t_end 0.05 \
  --outdir examples/rolling
```
...
接触線上の流体粒子に静摩擦・動摩擦を適用するモデルです。`surface_tension_enabled=1` と組み合わせて使用します。

| パラメータ | デフォルト値 | 説明 |
|---|---|---|
| `friction_enabled` | 0 | 摩擦モデルの有効化 (0: 無効, 1: 有効) |
| `friction_delta_theta` | 30.0 | 静摩擦: 臨界接触角偏差 Δθ_c [deg] |
| `dynamic_friction_alpha` | 600.0 | 動摩擦: 抵抗係数 α (論文値はl0=0.2mm向け; l0に応じて調整が必要) |
| `friction_velocity_threshold` | 0.01 | 静/動摩擦の切り替え臨界速度 v_c [m/s] |

**静摩擦**: 粒子速度 |v| ≤ v_c のとき、接触角偏差 |θ_i - θ_s| ≤ Δθ_c なら仮速度をゼロに固定。
**動摩擦**: 粒子速度 |v| > v_c のとき、接触線法線方向の速度成分を Δv_n = -dt·α·ν·(2D/n0λ)·Σw·v_n で減衰。

**壁法線推定**: 最近傍壁粒子 j* の近傍壁粒子群の加重平均から j* における外向き法線を推定。平面壁・球面壁・任意曲面に対応。

**接触線検出**: 標準の `on_surface` フラグは壁粒子込みの粒子数密度を使うため接触線で誤検出する。代わりに「流体のみの局所粒子数密度 n_f < surface_threshold × n0」かつ「WALL粒子が re_st 以内に存在」で判定する。
...
- Khayyer, A. & Gotoh, H. (2008). Development of CMPS Method for Accurate Water-Surface Tracking in Breaking Waves. *Coastal Engineering Journal*, 50(2), 179–207.
...
- Hattori, T. & Koshizuka, S. (2019). Numerical simulation of droplet behavior on an inclined plate using the Moving Particle Semi-implicit method. *Mechanical Engineering Journal*, 6(5), 19-00204. https://doi.org/10.1299/mej.19-00204
