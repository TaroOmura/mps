#!/usr/bin/env python3
"""
無重力下の液滴オシレーション問題の初期条件ファイルを生成するスクリプト (3D版)

直方体ブロックの流体粒子を生成。壁・ダミー粒子は使用しない。
表面張力が有効なとき、直方体→球への変形・振動が起こる。
...
def main():
    parser = argparse.ArgumentParser(description="液滴オシレーション問題の初期条件生成 (3D)")

    # 粒子設定
    parser.add_argument("--l0",  type=float, default=0.002,  help="粒子間距離 [m]")
    parser.add_argument("--nx",  type=int,   default=41,     help="x方向の粒子数")
    parser.add_argument("--ny",  type=int,   default=41,     help="y方向の粒子数")
    parser.add_argument("--nz",  type=int,   default=41,     help="z方向の粒子数")
    ...
    for iz in range(nz):
        for iy in range(ny):
            for ix in range(nx):
                ...
                y = iy * l0 - cy
                z = iz * l0 - cz
                particles.append((x, y, z, 0.0, 0.0, 0.0, 0))  # type=0: FLUID
    ...
    # Rayleigh 振動周波数 (3D, l=2 球面モード)
    # ω² = l(l-1)(l+1) σ / (ρ R³)  l=2 → 6σ/(ρ R³)
    omega = math.sqrt(6.0 * args.sigma / (args.density * R_eq ** 3))
    T_rayleigh = 2.0 * math.pi / omega

    # ドメイン: 液滴が動いても出ないよう十分大きく (±5 * R_eq)
    margin = 5.0 * R_eq
    domain_half = max(side_x, side_y, side_z) / 2.0 + margin

    print(f"Generated particles: {n_total} fluid (no wall/dummy)")
    print(f"Cuboid block:        {side_x*1e3:.1f} x {side_y*1e3:.1f} x {side_z*1e3:.1f} mm  "
          f"(nx={nx}, ny={ny}, nz={nz}, l0={l0*1e3:.2f} mm)")
    print(f"Equivalent sphere R: {R_eq*1e3:.2f} mm")
    print(f"Capillary dt limit:  {dt_cap*1e3:.3f} ms  (dt={args.dt*1e3:.3f} ms)")
    print(f"Rayleigh l=2 mode:   f={omega/(2*math.pi):.2f} Hz  "
    ...
    with open(particle_path, "w") as f:
        f.write("# MPS 3D Droplet Oscillation - Initial Particle Configuration\n")
        f.write(f"# Cuboid block: nx={nx}, ny={ny}, nz={nz}, l0={l0} m\n")
        f.write(f"# Equivalent sphere radius: {R_eq:.6f} m\n")
        f.write(f"# Rayleigh l=2 period: {T_rayleigh*1e3:.2f} ms\n")
        f.write(f"{n_total}\n")
    ...
    with open(param_path, "w") as f:
        f.write("# MPS 3D Droplet Oscillation Parameters\n")
        f.write("#\n")
        f.write("# Particle\n")
        ...
        f.write(f"domain_z_min         {-domain_half:.6f}\n")
        f.write(f"domain_z_max         { domain_half:.6f}\n")
        f.write("#\n")
        f.write("# Output\n")
    ...
    with open(cal_path, "w") as f:
        f.write("# MPS 3D Droplet Oscillation Control File\n")
