#!/usr/bin/env python3
"""
液滴濡れ問題の初期条件ファイルを生成するスクリプト (3D版)

gen_droplet.py の直方体流体ブロックに、底面の壁粒子・ダミー粒子を追加した構造。
左右・前後・上面は開放。底面のみ壁4層＋ダミー4層 (XZ平面)。
y軸が鉛直方向 (gravity_y = -9.81)。
...
def main():
    parser = argparse.ArgumentParser(description="液滴濡れ問題の初期条件生成 (3D)")
    ...
    parser.add_argument("--floor_extra",  type=int, default=20, help="床のXZ方向の追加マージン [粒子数]")
    ...
    # 流体粒子: 直方体ブロック、底面が y = gap_l0 * l0 になるよう配置
    for iz in range(nz):
        for iy in range(ny):
            for ix in range(nx):
                x = ix * l0 - cx
    ...
    # 壁・ダミー粒子: 底面のみ (XZ平面)、x・z範囲は流体ブロック幅 + 両端 (wl+dl+floor_extra) 層分
    ...
    print(f"Wall: {wl} layers, Dummy: {dl} layers (bottom only, XZ plane)")
    ...
    with open(particle_path, "w") as f:
        f.write("# MPS 3D Wetting - Initial Particle Configuration\n")
    ...
    with open(param_path, "w") as f:
        f.write("# MPS 3D Wetting Parameters\n")
        f.write("#\n")
        f.write("# Particle\n")
    ...
    with open(cal_path, "w") as f:
        f.write("# MPS 3D Wetting Control File\n")
