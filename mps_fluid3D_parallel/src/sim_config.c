...
void config_set_defaults(SimConfig *config)
{
    ...
    config->wetting_angle_SL         = 90.0;

    config->domain_min[0] = 0.0;
    config->domain_min[1] = 0.0;
    config->domain_min[2] = 0.0;
    config->domain_max[0] = 0.4;
 ...
 * value が '/' で始まる絶対パスならそのまま使用、
 * 相対パスなら cal_path のディレクトリと結合する。
 */
                         ...
                         char *out, size_t out_size)
{
    ...
    if (last_slash) {
        *(last_slash + 1) = '\0';  /* ディレクトリ部分のみ残す */
        snprintf(out, out_size, "%s%s", dir, value);
    } else {
        /* cal_path にディレクトリ部分がない (同じディレクトリ) */
        strncpy(out, value, out_size - 1);
        out[out_size - 1] = '\0';
    }
}
 ...
 *
 * ファイルパスは cal.txt のあるディレクトリからの相対パスとして解決される
 */
...
int config_load_params(const char *param_path, SimConfig *config)
{
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        } else if (strcmp(key, "gravity_z") == 0) {
            config->gravity[2] = atof(val_str);
        ...
        } else if (strcmp(key, "domain_z_min") == 0) {
