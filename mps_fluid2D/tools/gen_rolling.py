#!/usr/bin/env python3
"""
傾いた床での液滴転がりシミュレーション初期条件ファイルを生成するスクリプト (2D版)
...
def main():
    parser = argparse.ArgumentParser(description="傾斜面液滴転がりシミュレーション初期条件生成 (2D)")
    ...
    with open(particle_path, "w") as f:
        f.write("# MPS 2D Rolling Droplet - Initial Particle Configuration\n")
        ...
        f.write(f"# Floor extra: down={args.floor_extra_down}, up={args.floor_extra_up}\n")
    ...
    with open(param_path, "w") as f:
        f.write("# MPS 2D Rolling Droplet Parameters\n")
    ...
    with open(cal_path, "w") as f:
        f.write("# MPS 2D Rolling Droplet Control File\n")
